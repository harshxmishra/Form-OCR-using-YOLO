...
def main():
    ...
    for _ in range(7):
        for i in images:
            ...
            img = flip_images(img, 0)
            img = adjust_brightness(img, random.randrange(-60, 3))
            img = adjust_opacity(img, round(random.uniform(0.5,1), 3))
            ...
            cv.imwrite("/Form OCR/augment/z" + str(count) + i, img)
            count += 1
            # plt.imshow(img)
            # plt.show()
